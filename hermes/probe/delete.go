...
func DeleteFile(ctx context.Context, fileID int32, target *Target, client stiface.Client, logger *logger.Logger) (int32, error) {
	bucket := target.Target.GetBucketName()

	if fileID < minFileIDToDelete || fileID > maxFileIDToDelete {
		return fileID, fmt.Errorf("DeleteFile(%q, %q) failed; status %v: expected fileID %d to be within valid inclusive range: 11-50", bucket, fileID, metrics.ProbeFailed, fileID)
	}
